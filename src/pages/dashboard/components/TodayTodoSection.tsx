...
export default function TodayTodoSection({
  ...
  return (
    <>
      ...
      <div className={`rounded-xl border overflow-hidden relative transition-colors duration-300 ${showErrorFlash ? 'shake border-red-400 bg-red-50 dark:bg-red-900/20 dark:border-red-700' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700'}`}>
      ...
      <div className="px-6 py-4 bg-blue-50 dark:bg-blue-900/20 border-b border-blue-100 dark:border-blue-800/40">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">오늘의 할 일</h3>
      ...
      <div className="p-6">
        {isAuthenticated && loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="animate-pulse flex items-start gap-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
              ...
              {SAMPLE_PROBLEMS.map((problem) => (
                <div
                  ...
                  className={`block border rounded-lg p-4 cursor-default ${
                    problem.isSolved
                        ? 'bg-green-50 border-green-200 dark:bg-green-900/25 dark:border-green-800/50'
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          <ExternalLink className={`h-4 w-4 flex-shrink-0 ${
                            problem.isSolved ? 'text-green-500 dark:text-green-300' : 'text-gray-400 dark:text-gray-500'
                          }`} />
                        </div>
                    ...
                    {!problem.isSolved && (
                      <div className="flex-shrink-0 px-3 py-1.5 text-xs font-medium text-gray-600 dark:text-gray-200 bg-gray-100 dark:bg-gray-700 rounded-md flex items-center gap-1.5 border border-gray-200 dark:border-gray-600">
            ...
            {displayProblems.map((problem) => (
              <a
                ...
                className={`block border rounded-lg p-4 transition-colors group ${
                  problem.isSolved
                    ? 'bg-green-50 border-green-200 dark:bg-green-900/25 dark:border-green-800/50'
                    : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:border-gray-300 hover:bg-gray-50 dark:hover:border-gray-600 dark:hover:bg-gray-700'
                }`}
              >
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    <div className="flex-shrink-0 relative group/checkbox">
                      {problem.isSolved ? (
                        <>
                          <CheckCircle className="w-5 h-5 text-green-600 dark:text-green-300" />
                          <span className="absolute left-0 top-7 px-2 py-1 text-xs bg-gray-800 dark:bg-gray-900 text-white rounded whitespace-nowrap opacity-0 group-hover/checkbox:opacity-100 transition-opacity pointer-events-none z-10">
                            해결 완료
                          </span>
                        </>
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        <ExternalLink className={`h-4 w-4 flex-shrink-0 transition-colors ${
                          problem.isSolved ? 'text-green-500 dark:text-green-300' : 'text-gray-400 dark:text-gray-500 group-hover:text-blue-500 dark:group-hover:text-blue-400'
                        }`} />
                      </div>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        {problem.teamName} · #{problem.problemId}
                  ...
                  {!problem.isSolved && (
                    <button
                      ...
                      className="flex-shrink-0 px-3 py-1.5 text-xs font-medium text-gray-600 dark:text-gray-200 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 hover:text-gray-800 dark:hover:text-gray-100 disabled:bg-gray-100 dark:disabled:bg-gray-700 disabled:text-gray-400 dark:disabled:text-gray-500 rounded-md transition-colors flex items-center gap-1.5 border border-gray-200 dark:border-gray-600"
                    >
