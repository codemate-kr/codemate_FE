...
export default function TeamActivityBoard({
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg p-8 max-sm:p-6">
        <div className="flex items-center justify-center gap-2 text-gray-500 dark:text-slate-400">
          <div className="w-5 h-5 max-sm:w-4 max-sm:h-4 border-2 border-gray-300 dark:border-slate-700 border-t-blue-600 rounded-full animate-spin" />
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg">
      <div className="border-b border-gray-200 dark:border-slate-800 px-4 max-sm:px-3 pt-4 max-sm:pt-3">
        <div className="flex items-center justify-between mb-3 max-sm:mb-2">
          <div className="flex items-center gap-2 max-sm:gap-1.5">
            <Users className="h-5 w-5 max-sm:h-4 max-sm:w-4 text-blue-600 dark:text-blue-300" />
            <h3 className="text-sm max-sm:text-xs font-semibold text-gray-900 dark:text-gray-100">팀 활동</h3>
          </div>
        </div>
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-1.5 max-sm:gap-1 px-3 max-sm:px-2 py-2 max-sm:py-1.5 text-sm max-sm:text-xs font-medium rounded-t-lg transition-colors ${
                activeTab === tab.id
                  ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border-b-2 border-blue-600 dark:border-blue-400'
      ...
      <div className="p-4 max-sm:p-3">
        ...
        >
          {isActiveLoading && (
            <div className="absolute inset-0 z-10 bg-white/50 dark:bg-slate-900/50 pointer-events-none" />
          )}
          <div
            ref={contentRef}
