              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      { key: 'EASY' as ProblemDifficultyPreset, label: '쉬움', subtitleShort: '브Ⅰ~실Ⅲ', subtitleFull: '브론즈Ⅰ ~ 실버Ⅲ', color: 'bg-green-50 border-green-200 text-green-700' },
                      { key: 'NORMAL' as ProblemDifficultyPreset, label: '보통', subtitleShort: '실Ⅱ~골Ⅳ', subtitleFull: '실버Ⅱ ~ 골드Ⅳ', color: 'bg-blue-50 border-blue-200 text-blue-700' },
                      { key: 'HARD' as ProblemDifficultyPreset, label: '어려움', subtitleShort: '골Ⅲ~플Ⅴ', subtitleFull: '골드Ⅲ ~ 플래티넘Ⅴ', color: 'bg-red-50 border-red-200 text-red-700' }
                    ].map(preset => (
                      <label
                        key={preset.key}
                      ...
                      >
                        ...
                        <div className="text-center">
                          ...
                          <span className="text-xs text-gray-500 sm:hidden">{preset.subtitleShort}</span>
                          <span className="text-xs text-gray-500 hidden sm:inline">{preset.subtitleFull}</span>
                        </div>
                      </label>
                    ))}
                  </div>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="text-xs text-gray-700 space-y-1">
                    <p className="font-semibold text-gray-900">설정 요약</p>
                    {selectedDays.length > 0 ? (
                      <p>
                        매주{' '}
                        <span className="font-semibold text-blue-700">
                          {getSortedSelectedDays().map(day => weekDays.find(d => d.key === day)?.label.charAt(0)).join(', ')}
                        </span>
                        요일{' '}
                        <span className="font-semibold text-blue-700">오전 9시</span>에{' '}
                        <span className="font-semibold text-blue-700">
                          {selectedPreset === 'EASY' ? '쉬움(브론즈Ⅰ~실버Ⅲ)' :
                           selectedPreset === 'NORMAL' ? '보통(실버Ⅱ~골드Ⅳ)' :
                           selectedPreset === 'HARD' ? '어려움(골드Ⅲ~플래티넘Ⅴ)' :
                           selectedPreset === 'CUSTOM' && minProblemLevel && maxProblemLevel
                             ? `${getTierShortName(minProblemLevel)}~${getTierShortName(maxProblemLevel)}`
                             : '미설정'}
                        </span>
                        {' '}난이도{' '}
                        <span className="font-semibold text-blue-700">{problemCount}문제</span>가 전송됩니다.
                      </p>
                    ) : (
                      <p className="text-gray-500">추천 요일을 선택해주세요.</p>
                    )}
                  </div>
                </div>
              </div>
