  ...
  return (
    <div className={`flex items-center justify-between gap-3 p-2.5 rounded-lg transition-colors ${
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs text-gray-600 truncate font-medium" title={problemTitle}>
      ...
      <div className="flex items-center gap-1">
        ...
        <button
          ...
          className="p-2 text-gray-500 bg-gray-100 hover:bg-gray-200 rounded-lg transition-all"
          title="초기화"
        >
...
export function FloatingTimerWidget() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 animate-in slide-in-from-bottom-4 fade-in duration-300">
      <div className="bg-white rounded-xl shadow-xl border border-gray-200/80 overflow-hidden min-w-[280px] max-w-[340px]">
        {/* 헤더 */}
        <button
          onClick={() => setIsExpanded(!isExpanded)}
          className={`w-full px-4 py-3 flex items-center justify-between transition-all ${
            hasOverTime ? 'bg-red-50 hover:bg-red-100' :
            hasAnyPaused ? 'bg-orange-50 hover:bg-orange-100' :
